...
public class Board {
	...
	public void init() {
		...
		populateSpaces(Facade.getGameMode());
	...
	private void populateSpaces(int gameMode) {
		...
		case 0://Normal GameMode:
			...
			break;
		case 1:	//Christmas edition
			spaces[0] = new BlankSpace(banker, "Christmas Day");

			rates = new int[] {2,10,30,90,160,250};
			spaces[1] = new PropertySpace(banker, 
					"Family couch",propertyGroups[0], 60, 50, rates);

			spaces[2] = new CommunityChestSpace(banker, "Community Chest");

			rates = new int[] {4,20,60,180,320,450};
			spaces[3] = new PropertySpace(banker, 
					"Dad's Arm Chair", propertyGroups[0], 60, 50, rates);

			spaces[4] = new TaxSpace(banker, "Chocolate tax",true);

			rates = new int[] {200,25,50,100,200,0};
			spaces[5] = new Railroad(banker, "Red Toy Train Track", propertyGroups[8], 200, 0, rates);

			rates = new int[] {6,30,90,270,400,550};
			spaces[6] = new PropertySpace(banker, 
					"Kids's table", propertyGroups[1], 100, 50, rates);

			spaces[7] = new ChanceSpace(banker, "Present Time!");

			rates = new int[] {6,10,90,270,400,550};
			spaces[8] = new PropertySpace(banker, 
					"Adult Table", propertyGroups[1], 100, 50, rates);

			rates = new int[] {8,40,100,300,450,600};
			spaces[9] = new PropertySpace(banker, 
					"Snow Day Dr.", propertyGroups[1], 120, 50, rates);

			spaces[10] = new BlankSpace(banker, "Time out Corner");

			rates = new int[] {10,50,150,450,625,750};
			spaces[11] = new PropertySpace(banker, 
					"Igloo Way", propertyGroups[2], 140, 50, rates);

			tempReplaceU(150);
			spaces[12] = new Utility(banker, "Sugar Brick Block", propertyGroups[9], 150, 0, rates);

			rates = new int[] {10,50,150,450,625,750};
			spaces[13] = new PropertySpace(banker, 
					"Frosty's Lair", propertyGroups[2], 140, 50, rates);

			rates = new int[] {12,60,180,500,700,900};
			spaces[14] = new PropertySpace(banker, 
					"Icicle Cavern", propertyGroups[2], 160, 50, rates);

			rates = new int[] {200,25,50,100,200,0};
			spaces[15] = new Railroad(banker, "Blue Toy Train Track", propertyGroups[8], 200, 0, rates);

			rates = new int[] {14,70,200,550,750,950};
			spaces[16] = new PropertySpace(banker, 
					"Nut-Cracker Blvd.", propertyGroups[3], 180, 50, rates);

			spaces[17] = new CommunityChestSpace(banker, "Naughty or Nice?");

			rates =  new int[] {14,70,200,550,750,950};
			spaces[18] = new PropertySpace(banker, 
					"Candy Cane Avenue", propertyGroups[3], 180, 50, rates);

			rates = new int[] {16,80,220,600,800,1000};
			spaces[19] = new PropertySpace(banker, 
					"Ginger Bread Residence", propertyGroups[3], 200, 50, rates);

			spaces[20] = new BlankSpace(banker, "Mid-Day Nap.");

			rates = new int[] {18,90,250,700,875,1050};
			spaces[21] = new PropertySpace(banker, 
					"Roast Ham Place", propertyGroups[4], 220, 50, rates);

			spaces[22] = new ChanceSpace(banker, "Present Time!");

			rates = new int[] {19,90,250,700,875,1050};	
			spaces[23] = new PropertySpace(banker, 
					"Red Candy Cane Forest", propertyGroups[4], 220, 50, rates);

			rates = new int[] {20,100,300,750,925,1100};
			spaces[24] = new PropertySpace(banker, 
					"Green Candy Cane Forest", propertyGroups[4], 240, 50, rates);

			rates = new int[] {200,25,50,100,200,0};
			spaces[25] = new Railroad(banker, "Green Toy Train Track", propertyGroups[8], 200, 0, rates);

			rates = new int[] {22,110,330,800,975,1150};
			spaces[26] = new PropertySpace(banker, 
					"Parent's Room", propertyGroups[5], 260, 50, rates);

			rates = new int[] {22,110,330,800,975,1150};
			spaces[27] = new PropertySpace(banker, 
					"Brother's Room", propertyGroups[5], 260, 50, rates);

			tempReplaceU(150);
			spaces[28] = new Utility(banker, "Egg-Nog Fountain", propertyGroups[9], 150, 0, rates);

			rates = new int[] {24,120,360,850,1025,1200};
			spaces[29] = new PropertySpace(banker, 
					"Toy Jungle", propertyGroups[5], 280, 50, rates);

			spaces[30] = new GotoJailSpace(banker, "Get in Trouble");

			rates = new int[] {26,130,390,900,1100,1275};
			spaces[31] = new PropertySpace(banker, 
					"Santa's Slay", propertyGroups[6], 300, 50, rates);

			rates = new int[] {26,130,390,900,1100,1275};
			spaces[32] = new PropertySpace(banker, 
					"Present Lane", propertyGroups[6], 300, 50, rates);

			spaces[33] = new CommunityChestSpace(banker, "Naughty or Nice?");

			rates = new int[] {28,150,450,1000,1200,1400};
			spaces[34] = new PropertySpace(banker, 
					"Snow Fort Mark", propertyGroups[6], 320, 50, rates);

			rates = new int[] {200,25,50,100,200,0};
			spaces[35] = new Railroad(banker, "White Toy Train Track", propertyGroups[8], 200, 0, rates);

			spaces[36] = new ChanceSpace(banker, "Naughty or Nice?");

			rates = new int[] {35,175,500,1100,1300,1500};
			spaces[37] = new PropertySpace(banker, 
					"Under the Christmas Tree", propertyGroups[7], 350, 50, rates);

			spaces[38] = new TaxSpace(banker, "Milk Tax",false);

			rates = new int[] {50,200,600,1400,1700,2000};
			spaces[39] = new PropertySpace(banker, 
					"Santa's Workshop", propertyGroups[7], 400, 50, rates);
			break;
		default:
			break;
		}
