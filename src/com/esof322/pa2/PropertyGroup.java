...
public class PropertyGroup {
    ...
    public void checkMonopoly(int i, int c) {//int i represents the player's piece number, int c represents property color
        ...
        for(int j = 0; j <  properties[c].length; j++) {
        	if((i == properties[c][j].checkOwner())&&(!properties[c][j].isMortgaged())) { //Monopoly is only recognized if none of the properties are mortgaged.
        		owned++;
        	}
        }
        if(k <= owned) {
        	makeMonopoly(properties[c],true);
        }
        makeMonopoly(properties[c],false);
    }
    
    public void makeMonopoly(PropertySpace[] p, boolean b) {
